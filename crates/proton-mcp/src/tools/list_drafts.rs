//! `list_drafts` — list draft messages (paged).
...
#[tool_router(router = list_drafts_router, vis = "pub(crate)")]
impl ProtonMail {
    #[tool(
        name = "list_drafts",
        description = "List draft messages (paged). Returns total count and message metadata."
    )]
